...
ユーザーの好みに合わせて **2つのテーマ** を切り替え可能にする。
...
### テーマ1: モダン（デフォルト・初心者向け）

今風のUI/UXで、優しくてスタイリッシュな見た目。
...
ファミコン/ゲームボーイ時代を彷彿とさせるレトロゲーム風UI。
...
| 要素 | ライト (ゲームボーイ風) | ダーク (ファミコン風) |
|------|----------------------|---------------------|
| メイン背景 | #9BBC0F (GB緑) | #000000 (真っ黒) |
| サブ背景 | #8BAC0F | #0F0F0F |
| エディタ背景 | #306230 | #000020 (紺) |
| テキスト | #0F380F (濃緑) | #FCFCFC (白) |
| アクセント | #0F380F | #00FF00 (緑) |
| 成功 | #0F380F | #00FF00 |
| 警告 | #8BAC0F | #FFFF00 |
| エラー | #306230 | #FF0000 |
| キャラ吹き出し | #9BBC0F | #202020 |
| キーボード | #8BAC0F | #101010 |
| ボーダー | 2px solid #0F380F | 2px solid #FCFCFC |
...
│  8ビットテーマの特徴:                                   │
│                                                         │
│  ✅ ピクセルフォント（Press Start 2P など）            │
│  ✅ ドット絵のにゃぱん（16x16 or 32x32）               │
│  ✅ チップチューン風SE（ピコピコ音）                   │
│  ✅ CRTスキャンライン効果                              │
│  ✅ カクカクしたアニメーション（steps(4)）             │
│  ✅ ピクセル単位のボーダー                             │
│  ✅ 「PRESS START」風のボタン                          │
│  ✅ HPバー風の進捗表示                                 │
│                                                         │
...
│  └─────────────────────────────────────────────────┘   │
│                                                         │
│  フォント: "Press Start 2P", "DotGothic16"             │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
| 演出 | 説明 |
|------|------|
| 起動画面 | 「にゃぱん PRESENTS」のレトロ風スプラッシュ |
| 章クリア | ファンファーレ音 + 「STAGE CLEAR!」表示 |
| レベルアップ | 経験値バーがMAXになると「LEVEL UP!」 |
| コマンド成功 | 「ピコッ」というSE |
| コマンド失敗 | 「ブッ」というSE |
| 実績解除 | 「チャラララッチャッチャー♪」風SE |
...
const themes = {
  ...
  retro8bit: {
    ...
    borderRadius: '0',
    animation: 'steps(4) 0.2s'
  }
};

const useTheme = () => {
  const [themeStyle, setThemeStyle] = useState('modern'); // 'modern' | 'retro8bit'
  const [brightness, setBrightness] = useState('system'); // 'light' | 'dark' | 'system'
  const [soundEnabled, setSoundEnabled] = useState(true); // 8ビットテーマ用
...
/* カクカクアニメーション */
.theme-retro8bit .nyapan {
  animation: bounce 0.4s steps(4) infinite;
}
...
| チートシート印刷 | PDFでダウンロード可能なコマンド一覧 | 高 |
| キーボード音 | タイプ音のフィードバック | 低 |
| にゃぱん着せ替え | 季節衣装など | 低 |
| サバイバルモード | 間違えたら最初から | 中 |
| ランダム出題 | 復習モードでシャッフル | 中 |
...
| 機能 | 説明 |
|------|------|
| オンボーディング | 初回訪問時の導入ツアー（にゃぱんが案内） |
| サイト内Vimキー操作 | `j`で下スクロール、`k`で上スクロール等 |
| 音声ガイド | にゃぱんの声でセリフを読み上げ（オプション） |

#### 実用・便利機能
| 機能 | 説明 |
|------|------|
| 設定エクスポート/インポート | 進捗データのバックアップ/復元 |
| オフライン対応（PWA） | 電波なくても学習可能 |
| Vimrc生成 | 学んだ内容に基づいた設定ファイル出力 |

#### アクセシビリティ
| 機能 | 説明 |
|------|------|
| 色覚対応 | 色だけでなく形・アイコンでも状態を表示 |
| フォントサイズ調整 | 読みやすさのカスタマイズ |
| キーボードのみ操作 | マウスなしでも全機能使える |

#### コミュニティ・継続性
| 機能 | 説明 |
|------|------|
| フィードバック機能 | 「この説明わかりにくい」を送信できる |
| 学習リマインダー | 通知で「復習の時間だよ」（オプション） |
| 学習仲間機能 | フレンドと進捗を共有・競争 |
| ユーザーアカウント | クラウド同期 |
| コミュニティ機能 | Tips共有 |

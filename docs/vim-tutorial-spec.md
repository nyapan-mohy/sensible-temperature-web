...
### テーマ1: パステル（デフォルト・初心者向け）

黒い画面への恐怖を和らげる、優しい色合い。
...
実際のVim/ターミナルに近い、本格的な見た目。

| 要素 | ライト (Solarized風) | ダーク (クラシック) |
|------|---------------------|-------------------|
| メイン背景 | #FDF6E3 (Solarized Light) | #1A1A1A (真っ黒) |
| サブ背景 | #EEE8D5 | #0D0D0D |
| エディタ背景 | #FDF6E3 | #000000 |
| テキスト | #657B83 | #00FF00 (グリーン) |
| アクセント | #268BD2 (青) | #00FF00 |
| 成功 | #859900 | #00FF00 |
| 警告 | #B58900 | #FFFF00 |
| エラー | #DC322F | #FF0000 |
| キャラ吹き出し | #EEE8D5 | #333333 |
| キーボード | #EEE8D5 | #1A1A1A |
| カーソル | - | ブロックカーソル点滅 |
...
│  ターミナルテーマ選択時の追加演出:                      │
│                                                         │
│  ✅ エディタ部分にスキャンライン効果（CRT風）          │
│  ✅ タイプ時に「カチャカチャ」音（オプション）         │
│  ✅ カーソルがブロック型で点滅                         │
│  ✅ にゃぱんがドット絵風になる（オプション）           │
│  ✅ 起動時に「Vim - Vi IMproved」風のスプラッシュ      │
│                                                         │
│  ┌─────────────────────────────────────────────────┐   │
│  │  $ vim tutorial.txt                             │   │
│  │  ▊                                              │   │
...
const themes = {
  ...
  terminal: {
    light: { /* ターミナルライト (Solarized) */ },
    dark: { /* ターミナルダーク (Classic) */ }
  }
};

const useTheme = () => {
  const [themeStyle, setThemeStyle] = useState('pastel'); // 'pastel' | 'terminal'
  ...
  return { themeStyle, setThemeStyle, brightness, setBrightness, currentTheme };
};
```
